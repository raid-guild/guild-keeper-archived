module.exports = {
    name: "create-raid-rip",
    description: "Starts a raid or rip channel.",
    execute(message, args, botID, MEMBER_ROLE_ID) {
        if (args.length < 4)
            return message.channel.send(
                "Some args are missing! Use **!keeper help**."
            );

        let type = args[1] === "create-raid" ? "raid" : "rip";
        ...
        if (!pattern.test(proposal))
            return message.channel.send(
                `Argument 4 - [**${proposal}**] doesn't seem like a valid proposal link!`
            );
        ...
        let welcomeMessage = `Welcome & attention! A **${
            type === "rip" ? "RIP" : "RAID"
